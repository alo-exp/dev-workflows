...
**Current version: v0.20.0** — Composable Paths Architecture. Redesigned from fixed-pipeline workflows into 18 dynamically composable paths (PATH 0-17). The `/silver` router now composes path chains based on context, tracked by WORKFLOW.md. Includes artifact-review-assessor, 4-tier anti-stall, dual-mode quality gates, 3-tier silver-fast triage, WORKFLOW.md-first hook enforcement, and silver:migrate for existing projects. Previous: v0.19.1 hooks exit silently in non-SB directories; v0.18.1 adds configurable review depth, review analytics, and cross-artifact consistency.
