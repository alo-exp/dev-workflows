...
var IDX = [
  ...
  { page:'Core Concepts', url:'/help/concepts/', anchor:'hooks',
    ...
    text:'Hooks are shell scripts Claude Code runs after every tool use PostToolUse. Four hooks: skill tracker compliance status stage enforcer completion audit. Block git commit push deploy if required skills missing. Cannot be overridden by Claude reasoning.' },
  { page:'Core Concepts', url:'/help/concepts/', anchor:'hooks',
  ...
  { page:'Dev Workflow', url:'/help/dev-workflow/', anchor:'review-loop',
    title:'Review loop enforcement — must reach approved',
    text:'Every review loop must iterate until reviewer explicitly returns approved. Never stop because issues are minor or close enough. Maximum 3 iterations before surfacing to user. Must reach maximum not stop early. Counts as approved only when reviewer outputs approved.' },
  ...
  { page:'DevOps Workflow', url:'/help/devops-workflow/', anchor:'execute',
    title:'Step 7 — IaC execute — lowest environment first',
    text:'Wave-based execution applies to lowest environment first dev verify health then move to next. Never apply to production before verifying in staging. Plan dry-run confirm before apply. Verify resource health after apply. Atomic commit per task.' },
